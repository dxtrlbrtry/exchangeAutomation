...
export async function sendMail(user, message) {
    ...
    await request(options)
        .then(resp => {
            if (resp.statusCode != 202) { throw 'Status code was not OK'; }
        })
...
export async function waitForEmail(user, message) {
    ...
    try {
        while(lastSentEmail == null || !lastSentEmail.body.includes(message.body)) {
            ...
            await user.getMail(mailFolders.SENTITEMS, '?$top=5&orderby=receivedDateTime%20desc')
                .then(emails => {
                    if (emails.length > 0) {
